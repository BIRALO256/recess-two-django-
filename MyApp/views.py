...
def patient_dashboard(request):
    appointments = Appointment.objects.all().filter(
        patientId=request.user.id).order_by('-id')[:1]
...
def patient_invoice(request):
    ...
    dischargeDetails = PatientDischarge.objects.all().filter(
...
def patient_book_appointment(request):
    ...
    if request.method == 'POST':
        ...
        if appointmentForm.is_valid():
            ...
            appointment.doctorName = User.objects.get(
...
def doctor_dashboard(request):
    ...
    patientcount = Patient.objects.all().filter(
        status=True, assignedDoctorId=request.user.id).count()
    appointmentcount = Appointment.objects.all().filter(
        status=True, doctorId=request.user.id).count()
    patientdischarged = PatientDischarge.objects.all().distinct().filter(
        assignedDoctorName=doctor.get_name).count()
    mydict = {
...
def doctor_view_discharged_patient(request):
    doctor = Doctor.objects.get(user_id=request.user.id)
    dischargedpatients = PatientDischarge.objects.all(
    ).distinct().filter(assignedDoctorName=doctor.get_name)
...
def DoctorLogin(request):
    if request.method == "POST":
        # name from the html input is passed in brackets
        username = request.POST.get('username')
        password = request.POST.get('password')
...
def discharge_patient(request, pk):
    ...
    if request.method == 'POST':
        ...
        PatDisc.total = (int(request.POST['roomCharge'])*int(d))+int(request.POST['doctorFee'])+int(
...
def admin_add_appointment(request):
    ...
    if request.method == 'POST':
        ...
        if appointmentForm.is_valid():
            ...
            appointment.patientName = User.objects.get(
